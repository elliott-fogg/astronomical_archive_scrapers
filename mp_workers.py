...
# Use a file that is only written to by the main process, and is read-only to all of the subprocesses. If the file is empty, wait and try again. If the file says "KILL", kill the process off.
...
def download_chart_files(chart_type, country_list, date_list, queue, name):
    ...
    for country in country_list:
        ...
        output_folder = pjoin(base_dir, "country_" + country)
...
def get_artists_by_genre( (sp, genre, pipe) ):
    base_request = sp.search(q="genre:'{}'".format(genre), type="artist")
    if not base_request:
        return 1
        
    total_artists = base_request['artists']['total']
    if total_artists > 2000:
        return 1
    
    # Set up the output folder
    output_folder = pjoin("data", "artists_by_genre")
    check_folder(output_folder)
    
    # Download artists for this genre
    request = base_request
    output_list = []
    
    while True:
        if sf.get_kill_status():
            break
        
        for artist in request['artists']['items']:
            artist_data = {"id": artist['id'], "name": artist['name'], "genres": tuple(artist['genres'])}
            output_list.append(artist_data)
        
        if request['artists']['next']:
            request = sp.next(request['artists'])
        else:
            break
    
    output_file = pjoin(output_folder, "{}.pkl".format(genre))
    pickle_dump(output_file, output_list)
    queue.put(genre)
